from api_tests.test_data import expected_airports, airports_distance


def test_verify_airport_count(get_airports_data):
    airports = get_airports_data["data"]
    print(len(airports))
    assert len(airports) == 30, f"Expected 30 airports but got {len(airports)}"
...
def test_distance_between_two_airports(post_distance_between_airports):
    ...
    assert kilometers > 400, f"Distance should be greater than 400 km, but got {kilometers} km"
